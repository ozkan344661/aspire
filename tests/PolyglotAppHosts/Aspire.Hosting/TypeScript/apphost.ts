...
const project = await builder.addProject("myproject", "./src/MyProject", { launchProfileName: "https" });
...
const builtConnectionString = await builder.addConnectionString("customcs", {
    build: async (connectionStringBuilder) => {
        const _isEmpty: boolean = await connectionStringBuilder.isEmpty();
...
const expressionConnectionString = await builder.addConnectionString("exprcs", { expression: expr });
...
await pipeline.configure(async (configContext) => {
    ...
    const _builderTaggedSteps = await builderPipeline.stepsByTag("custom-build");
});
